...
class AES_Dec_FIFOTestVectors_MultiEnc(dataWidth: BitCount) extends Component {
  ...
  val io = new Bundle {
    ...
    val out_ct_vld = out Bool ()
    val out_block = out Bits (config.dataWidth)
    val out_gold_tag = out Bits (config.dataWidth)
  ...
  val ct_vldReg: Bool = Reg(Bool) init (False)
  ...
  val tagReg: Bits = Reg(Bits(128 bits)) init (0)
  ...
  val inputFifo2 = StreamFifo(
    dataType = (AES_DecCmd(config)),
    depth = 16
  )

  val streamDataInput1, streamDataOutput1 = Stream(AES_DecCmd(config))
  inputFifo2.io.push << streamDataInput1
  inputFifo2.io.pop >> streamDataOutput1

  streamDataOutput1.ready := io.in_ready
  io.out_block := streamDataOutput1.payload.block
  io.out_gold_tag := streamDataOutput1.payload.tag_gold
  ...
  io.out_ct_vld := streamDataOutput1.payload.ct_vld
      ...
      whenIsActive {
        when(streamDataInput1.ready) {
          when(counterReg === 0) {
            ...
            keyReg := key0.asBits
            tagReg := tag0.asBits // todo pop the gold_tag together with initial vector and the key
            vector_vldReg := True
            key_vldReg := True
            ct_vldReg := False
            last_wordReg := False
        ...
        whenIsActive {
          ...
          when(io.in_aesoutput_dataValid) {
            when(counterRegOut === 0) {
              when(io.in_aesoutput_data(127 downto 0) === plainData10.asBits) {
                passReg := True
              } otherwise {
                passReg := False
              }
            ...
            when(counterRegOut === 1) {
              when(io.in_aesoutput_data(127 downto 0) === plainData20.asBits) {
                passReg := True
              } otherwise {
                passReg := False
              }
            ...
            when(counterRegOut === 2) {
              when(io.in_aesoutput_data(127 downto 0) === plainData30.asBits) {
                passReg := True
              } otherwise {
                passReg := False
              }
            ...
            when(counterRegOut === 3) {
              when(io.in_aesoutput_data(127 downto 0) === plainData40.asBits) {
                passReg := True
              } otherwise {
                passReg := False
              }
