...
class AES_Dec_TestVectors(dataWidth: BitCount) extends Component {
  ...
  val sm = new StateMachine {
    ...
    val sPrepare: State = new State {
      whenIsActive {
        ...
        tagReg(127 downto 0) := tag0.asBits
        vector_rdyReg := True
    ...
    val sEncValues: State = new State {
      whenIsActive {
        when(io.io.cmd.ready === True) {
          ...
          when(counterReg === 0) {
            ct_rdyReg := True
            dataReg(127 downto 0) := cipher10.asBits
            sizeReg := B("1111")
          }
          when(counterReg === 1) {
            ct_rdyReg := True
            dataReg(127 downto 0) := cipher20.asBits
          }
          when(counterReg === 2) {
            ct_rdyReg := True
            dataReg(127 downto 0) := cipher30.asBits
          }
          when(counterReg === 3) {
            ct_rdyReg := True
            dataReg(127 downto 0) := cipher40.asBits
            lastReg := True
            vldReg := False
            goto(sComplete)
          }
  ...
  io.io.cmd.vector_vld := vector_rdyReg
  io.io.cmd.ct_vld := ct_rdyReg
  io.io.cmd.aad_vld := aad_rdyReg
  io.io.cmd.key_vld := key_rdyReg
  io.io.data_size := sizeReg.asUInt
  io.io.cmd.payload.last_word := lastReg
  io.io.cmd.payload.tag_gold := tagReg
      ...
      whenIsActive {
        ...
        when(io.io.rsp.data_vld) {
          when(counterRegOut === 0) {
            when(io.io.rsp.payload.Out_data(127 downto 0) === plainData10.asBits) {
              passBlockReg := True
            } otherwise {
              passBlockReg := False
            }
          ...
          when(counterRegOut === 1) {
            when(io.io.rsp.payload.Out_data(127 downto 0) === plainData20.asBits) {
              passBlockReg := True
            } otherwise {
              passBlockReg := False
            }
          ...
          when(counterRegOut === 2) {
            when(io.io.rsp.payload.Out_data(127 downto 0) === plainData30.asBits) {
              passBlockReg := True
            } otherwise {
              passBlockReg := False
            }
          ...
          when(counterRegOut === 3) {
            when(io.io.rsp.payload.Out_data(127 downto 0) === plainData40.asBits) {
              passBlockReg := True
            } otherwise {
              passBlockReg := False
            }
